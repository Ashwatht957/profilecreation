...
def parse_resume():
    if request.method == 'GET':
        return jsonify({
            "message": "⚠️ This endpoint only accepts POST requests with JSON data.",
            "example_format": {
                "zip_url": "https://example.com/yourfile.zip",
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                resume_file = next(
                    (f for f in zip_ref.namelist() if f.endswith(('.txt', '.pdf'))),
                    None
                )
